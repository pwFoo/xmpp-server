...
if [ -f /cert/acme.json ]; then
    ...
    mv /tmp/certs/certs/${TURN_REALM}.crt /tmp/certs/private/${TURN_REALM}.key /etc/coturn/certs/
fi

# paths to conntecion certificates for encryption
echo "cert=/etc/prosody/certs/$TURN_REALM.key" #cert.pem
echo "pkey=/etc/prosody/certs/$TURN_REALM.crt" #privkey.pem
...
echo "no-loopback-peers" >> /etc/turnserver.conf
